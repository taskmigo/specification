...
| LANG-001–LANG-004       | Inspect Taskmigo-owned Policy IR and test complete boolean returns, immutable `const` bindings, conditional semantics, and early return.                  |
...
| QUERY-001–QUERY-003     | Validate residual queryability across control flow, callable/utility-function exclusion, and persistence independence.                                   |
...
1. Request Authorization SHALL evaluate a valid direct TPL policy body using known `principal` and `request` values.
2. Object Authorization SHALL partially evaluate a valid direct TPL policy body with unknown `object`, validate residual queryability, and apply the lowered authorization predicate before pagination as required by the [Authorization Object requirements](../002.%20Authorization/04-functional-and-behavioral-requirements.md#43-object-authorization-and-shared-filter-ast).
